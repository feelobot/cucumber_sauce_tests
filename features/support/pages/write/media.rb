...
class Media 
  ...
  def Media.drag_and_drop(source,target)
    drop_points = $wait.until { $driver.find_elements(:class => 'ui-droppable') }
    target =  drop_points[target]
    raise "Not enough paragraphs to add media" if drop_points.length < 2
    $driver.action.drag_and_drop(source, target).perform
  end
  def Media.search
  ...
  def Media.get_slots
    slots = $wait.until { $driver.find_elements(:class => 'media_area_controls') }
  end

  def Media.expand(number)
    Media.get_slots[number].find_element(:class => 'expand').click    
    sleep 2
  end

  def Media.contract(number)
    Media.get_slots[number].find_element(:class => 'contract').click     
    sleep 2 
  end

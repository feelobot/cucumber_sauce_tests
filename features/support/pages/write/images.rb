...
class Images
  ...
  def edit(number,paragraph, caption=nil,full_width=false)
    s = Time.now.to_i
    edit_button = $wait.until { $driver.find_elements(:class => 'edit-photo')[number] }
    edit_button.location_once_scrolled_into_view
    edit_button.click
    cropper = ImageCropper.new
    cropper.set_caption(caption) unless caption == nil
    cropper.toggle_full_width unless full_width == false
  ...
  def verify_search_results
    begin
      $long_wait.until { @image_source_getty = $driver.find_element(:xpath => '//*[@id="discovery-search-results-getty"]/div/span').text }
      ...
      $first_img_result_id = result.attribute('data-getty_id') || result.attribute('data-presswire_id')
    rescue Selenium::WebDriver::Error::TimeOutError => e
      raise "Something Went Wrong With Image Search"
    end
    return result
  end
